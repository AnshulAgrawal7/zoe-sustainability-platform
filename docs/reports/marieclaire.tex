...
It should be noted that the project brief named Cronholm and G\"obel (2018) alongside Gregor and Jones (2007) as a form for design principles; that source is, however, not part of the project's literature corpus, and --- in keeping with the project's strict no-hallucination rule --- only Gregor and Jones (2007) is used here. Adopting the Cronholm and G\"obel formulation is noted as an open item for verification.
...
\textbf{DP2b (form and function, with a caution).} \emph{Use engagement mechanics --- immediate feedback, progress, illustrating relevance --- but not purely reward-based ones.} The positive part is justified by experimental evidence that such design features significantly raise willingness to engage (Yang \& Wu, 2025, Type~A) and by the mechanisms synthesised in a review (Krath et al., 2021, Type~B); the caution is justified by a review finding that reward-only gamification may evoke only short-term effects and may reduce participation quality (Thiel et al., 2016, Type~B). DP2b is therefore implemented as a \emph{deliberate, evaluable} design decision rather than as an unconditional feature.
...
DP2b & Engagement mechanics, but not purely reward-based & Yang \& Wu 2025 (A); Krath 2021 (B); Thiel 2016 (B)\\
...
\subsection{The design cycle: iterations}

The design cycle is reconstructed from the project's development log, which records seven iterations, each with the sub-problem addressed, the decision taken, its justification, and its status. \emph{Iteration~1} built the frontend MVP --- the action catalogue, SDG dashboard, transparency page, and participation page --- and made the early decision to externalise all data into a fallback layer so that an API could later replace it without rewriting the interface. \emph{Iteration~2} grounded the content in the real North Corfu context (the Antinioti Lagoon, the Erimitis peninsula, the Temploni landfill crisis, the Ionian Environment Foundation) and added a target-audience page, strengthening relevance. \emph{Iteration~3} was the full-stack step: a Node/Express/Prisma/SQLite backend, JWT authentication, and an administrative area realising the stakeholder requirement that the municipality can create and edit projects. \emph{Iteration~4} added internationalisation (English, Greek, German), dark mode, and Zustand state. \emph{Iteration~5} addressed accessibility to WCAG~2.1 AA with automated axe-core tests, grounded in the legal baseline of the EU Web Accessibility Directive. \emph{Iteration~6} added an end-to-end test suite. \emph{Iteration~7}, the most recent and deliberately additive, introduced the ``Get Involved'' page with accessible initiative tabs (DP1), the first realisation of the visitor-contribution principle (DP6), and a newsletter opt-in concept --- each with unit and accessibility tests. This sequence documents, in Hevner's sense, the tight loop of construction and assessment, and it shows the artefact maturing from a static prototype into a tested full-stack instantiation. Importantly, the iterations also record honest limitations --- for example, that several legacy pages still contain hard-coded text and are therefore not yet fully multilingual, and that the free submission of initiatives is not persisted --- which are themselves design knowledge for the next cycle and which the matrix carries forward as open items rather than concealing.

% TODO: Bild einfügen — Zeitstrahl der 7 Iterationen (aus DEVLOG.md)
\figplaceholder{Timeline of the seven iterations (from docs/DEVLOG.md)}
...
\vspace{0.5em}
\small\emph{AI-use reflection.} Generative AI tools supported literature triage, drafting, and the structuring of the matrix. All cited sources were verified against the original PDFs; uncertain bibliographic details are flagged in \texttt{docs/literature-index.md}, and the brief's suggested Cronholm and G\"obel (2018) form is noted as absent from the corpus. The author takes responsibility for the content.
