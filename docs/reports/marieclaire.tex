...
Two architectural decisions are design-relevant. First, the strict separation of the user interface from data access (components never call the network directly; all access goes through a service layer) realises the \emph{principle of implementation} that the prototype's static fallback data can be replaced by an API without rewriting the interface --- which is exactly what enabled the move from a static frontend to a full-stack artefact across iterations. Second, the decision \emph{not} to operate a production backend for the free submission of citizen initiatives in the beta is an explicit, conscious design decision, not an omission: processing citizens' personal data at scale would require privacy-by-design (Diamantopoulou et al., 2019) and raises the transparency--privacy tension documented by Paguay-Chimarro et al. (2025); it is therefore deferred to Future Work.

\subsection{The design cycle: iterations}

The design cycle is reconstructed from the project's development log. The artefact grew over seven iterations: an initial frontend MVP (catalogue, SDG dashboard, transparency, participation); the grounding of the content in the real North Corfu context and a target-audience page; a full-stack step adding the backend, authentication, and the administrative area; the internationalisation, dark-mode, and state-management step; the WCAG~2.1 AA accessibility step with automated axe-core tests; an end-to-end testing step; and, most recently, an additive ``Get Involved'' iteration that introduced initiative tabs, a visitor-contribution section (the first realisation of DP6), and a newsletter opt-in concept. Each iteration is logged with the sub-problem addressed, the decision taken, its justification, and its status --- documenting, in Hevner's sense, the tight loop of construction and assessment. Importantly, the iterations also record honest limitations (for example, that several legacy pages still contain hard-coded text and are therefore not yet fully multilingual), which are themselves design knowledge for the next cycle.
