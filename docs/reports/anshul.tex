...
\subsection{Demonstration protocol and cross-cutting observations}

The four scenarios share a common protocol that makes the demonstration reproducible and prepares the transition to evaluation. Each scenario fixes a \emph{persona} (a representative member of one target group), a \emph{precondition} (the state of the artefact and the entry point), a sequence of \emph{steps} (the concrete interaction path), and an \emph{expected observation} (what a viewer should see that evidences the design principle). For Scenario~A, the expected observation is that the visitor reaches a concrete, actionable contribution within a few clicks and across language versions; for Scenario~B, that an idea is submitted and acknowledged \emph{without} any registration step; for Scenario~C, that a non-expert can name the SDGs an action serves and read its impact while remaining aware that the figures are illustrative; and for Scenario~D, that the same task can be completed by keyboard alone and in any of the three languages with a visible focus indicator at every step.

Three cross-cutting observations emerge. First, honesty is built into the demonstration: each scenario names at least one limitation (non-persisted submissions, prototype impact data, partially internationalised legacy pages), which both respects the ``built versus conceived'' rule and pre-registers the risks that Phase~5 must probe. Second, the design principles interact: the visitor scenario (DP6) only works because the catalogue is central and grouped (DP1) and because the interface is multilingual (DP4), so the demonstration also shows the \emph{coherence} of the principle set rather than four isolated features. Third, the demonstration already surfaces measurable quantities --- click depth, number of steps, presence of focus indicators, completeness of translations --- that become the dependent variables of the evaluation, which is the subject of the next section.
