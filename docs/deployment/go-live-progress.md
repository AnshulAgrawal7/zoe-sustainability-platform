...
- 2026-06-07 — **#21 ✅**: Merge `claude/nightly-run` → `main` (Fast-Forward, `d8ff629..99987b4`) + Push zu `origin/main` erfolgreich (pre-push: Backend 37/37). Lokal == Remote. **Alle 🤖-Aufgaben (#17–#21) abgeschlossen.** Offen bleiben ausschließlich die 👤-Schritte aus [`to-do.md`](../../to-do.md) (Supabase/Render/Vercel-Accounts, Domain, Secrets, Rechtsdaten ausfüllen).
- 2026-06-07 — **#21 (Arbeit)**: DEVLOG **Iteration 9** ergänzt (Übersichtstabelle + Volleintrag + TP1-Statusfilter als erledigt markiert); `to-do.md` 🤖-Punkte abgehakt + Analytics/Monitoring-Zeilen ergänzt; `.gitignore`-Sweep (193 getrackte Dateien, keine Secrets/DBs/Binaries; defensive Einträge für `.claude/*.local.json`, Build-/Test-Caches ergänzt); Lint-Fix in `authController.ts` (ungenutztes `_maxAge`). Volle Verifikation grün: FE tsc + 20/20 + build, BE tsc + 37/37, ESLint 0, i18n-Parität 435/435/435. → danach Merge auf `main` + Push.
