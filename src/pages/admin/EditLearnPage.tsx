...
export default function EditLearnPage() {
    ...
    getLearningResource(id)
      .then((r) => {
        setForm({
          ...
          bodyEl: r.bodyEl,
          bodyDe: r.bodyDe,
          category: r.category as (typeof LEARN_CATEGORIES)[number],
          imageUrl: r.imageUrl ?? '',
          sourceNote: r.sourceNote ?? '',
          projectId: r.projectId ?? '',
        });
        try {
          setSelectedSdgs(JSON.parse(r.sdgIds) as number[]);
        } catch {
          setSelectedSdgs([]);
        }
      })
  ...
  async function handleDelete() {
    if (!id || !window.confirm(t('adminLearn.deleteConfirm'))) return;
    setLoading(true);
    try {
