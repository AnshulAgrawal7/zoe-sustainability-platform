...
export default function EditFeedPage() {
  ...
  async function saveAlt(img: AdminFeedImage) {
    try {
      await updateFeedImage(img.id, { altText: img.altText ?? '' });
    } catch {
      /* non-critical */
    }
  }

  function setImgAlt(imageId: string, value: string) {
    setImages((prev) =>
      prev.map((im) => (im.id === imageId ? { ...im, altText: value } : im))
    );
  }
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    type="text"
                    value={img.altText ?? ''}
                    onChange={(e) => setImgAlt(img.id, e.target.value)}
                    className={inputClass}
                  />
                  <button
                    type="button"
                    onClick={() => void saveAlt(img)}
                    className="inline-flex shrink-0 items-center gap-1 rounded-lg border border-gray-300 px-3 text-xs font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-600 dark:text-gray-300 dark:hover:bg-gray-700"
                  >
